...
double hour_angle; //in radians
double minute_angle; //in radians

double X_Rotation(int cur_x, int cur_y, double angle){
		double x = cos(angle)*(cur_x-CENTER_X) - sin(angle)*(cur_y-CENTER_Y) + CENTER_X;
		
		return x;
}
double Y_Rotation(int cur_x, int cur_y, double angle){
		double y = sin(angle)*(cur_x-CENTER_X) + cos(angle)*(cur_y-CENTER_Y) + CENTER_Y;
...
void Draw_Clock(){
  ...
  ST7735_XYplot(180,(int32_t *)CircleXbuf,(int32_t *)CircleYbuf, color);
	...
	ST7735_Line(CENTER_X,CENTER_Y,
				(int)X_Rotation(hour_x_coord,hour_y_coord,hour_angle),
					(int)Y_Rotation(hour_x_coord,hour_y_coord,hour_angle),color); //draw hour hand
				
	ST7735_Line(CENTER_X,CENTER_Y,
				(int)X_Rotation(minute_x_coord,minute_y_coord,minute_angle),
					(int)Y_Rotation(minute_x_coord,minute_y_coord,minute_angle),color);//draw minute hand x2>x1 and y1>y2
