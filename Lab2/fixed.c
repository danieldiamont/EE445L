...
void ST7735_sDecOut2(int32_t n)
{
	...
	if(n >= -9999 && n <= 9999)
	{
		...
		if(intsToPrint == 1)
			ST7735_OutChar(' ');//print out a space
		...
		{
			ST7735_OutChar(0x30 + integer%10);
			integer /= 10;
		}
		
		ST7735_OutChar('.');
		
		ST7735_OutChar(0x30 + fraction%10);//print the fraction characters
		ST7735_OutChar(0x30 + fraction/10);		
	}
...
{	
	...
	{		
		...
		ST7735_OutChar(' ');//print out space
		...
		{
			ST7735_OutChar(0x30 + integer%10);//print out the integer characters
			integer /= 10;
		}
		ST7735_OutChar('.');
		ST7735_OutChar(0x30 + fraction%10);//print out the fraction characters
		ST7735_OutChar(0x30 + fraction/10);
...
{	
	for(int x = 0; x < num; x++)//for all items in each buffer...
	{
		...
		{
			//draw four points
			ST7735_DrawPixel( ((bufX[x]-X_min)*127)/RangeX, 32+(((Y_max-bufY[x])*127)/RangeY), ST7735_BLUE);
			ST7735_DrawPixel( 1+((bufX[x]-X_min)*127)/RangeX, 32+(((Y_max-bufY[x])*127)/RangeY), ST7735_BLUE);
			ST7735_DrawPixel( ((bufX[x]-X_min)*127)/RangeX, 1+32+(((Y_max-bufY[x])*127)/RangeY), ST7735_BLUE);
			ST7735_DrawPixel( 1+((bufX[x]-X_min)*127)/RangeX, 1+32+(((Y_max-bufY[x])*127)/RangeY), ST7735_BLUE);
		}
