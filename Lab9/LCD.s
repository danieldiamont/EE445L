; LCD.s
; Student names: Daniel Diamont and Karim Sabar
...
check	LDR R1, =SSI0_SR_R
		LDR R2, [R1]
		AND R2, #0x10
		CMP R2, #0x00	;check if bit 4 is low 
		BEQ L1
		B check
		
L1		LDR R1, =DC		;clear DC
		LDR R3, [R1]
		AND R3, #0xBF	;clear bit 6
		STR R3, [R1]
		
		LDR R1, =SSI0_DR_R	;Write command to SSIO_DR_R
		STR R0, [R1]
		
check_1	LDR R1, =SSI0_SR_R
		LDR R2, [R1]
		AND R2, #0x10
		CMP R2, #0x00	;check if bit 4 is low 
		BEQ finish
...
check_2	LDR R1, =SSI0_SR_R
		LDR R2, [R1]
		AND R2, #0x02				;check bit 1
		CMP R2, #0x00
		BNE L2
		B check_2
