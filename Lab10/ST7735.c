...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor,uint16_t backgroundColor){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6, y*10, *pt, textColor, backgroundColor, 1);
    pt++;
    x = x+1;
...
void ST7735_OutUDec(uint32_t n,uint16_t color){
  ...
  ST7735_DrawString(StX,StY,Message,color,0xffff-color);
...
// Outputs: none
void ST7735_PlotPoint(int32_t y, uint16_t color){int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
...
// Position determined by ST7735_SetCursor command
// Color set by ST7735_SetTextColor
// Inputs: 8-bit ASCII character, uint16_t color
// Outputs: none
void ST7735_OutChar(char ch, uint16_t color){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    StY++; StX=0;
    if(StY>15){
      StY = 0;
    }
    ST7735_DrawString(0,StY,"                     ",color,0xffff-color);
    return;
  }
  ST7735_DrawCharS(StX*6,StY*10,ch,color,0xffff-color, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',color,0xffff-color, 1);
  }
  return;
}
//********ST7735_OutString*****************
...
// outputs: none
void ST7735_OutString(char *ptr,uint16_t color){
  while(*ptr){
    ST7735_OutChar(*ptr,color);
    ptr = ptr + 1;
  }
}
// ************** ST7735_SetTextColor ************************
